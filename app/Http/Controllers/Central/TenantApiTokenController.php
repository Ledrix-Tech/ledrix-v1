<?php

namespace App\Http\Controllers\Central;

use App\Http\Controllers\Controller;
use App\Models\Central\AuditLog;
use App\Models\Central\Tenant;
use App\Models\Central\TenantApiToken;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class TenantApiTokenController extends Controller
{
    public function store(Request $request, int $tenantId)
    {
        $tenant = Tenant::query()->findOrFail($tenantId);

        $validated = $request->validate([
            'name'       => ['required', 'string', 'max:100'],
            'abilities'  => ['nullable', 'string', 'max:500'],
            'expires_at' => ['nullable', 'date', 'after:today'],
        ]);

        $abilities = ['*'];
        if (! empty($validated['abilities'])) {
            $abilities = collect(explode(',', $validated['abilities']))
                ->map(fn ($a) => trim($a))
                ->filter()
                ->values()
                ->all() ?: ['*'];
        }

        [$plain, $record] = TenantApiToken::generate((int) $tenant->id, $validated['name'], $abilities);

        if (! empty($validated['expires_at'])) {
            $record->update(['expires_at' => $validated['expires_at']]);
        }

        $actor = Auth::guard('super_admin')->user();

        AuditLog::record(
            'tenant.api_token_created',
            $tenant->id,
            'super_admin',
            $actor?->id,
            $actor?->name,
            [
                'subject_type' => 'tenant_api_token',
                'subject_id'   => $record->id,
                'description'  => "Created API token \"{$record->name}\" for {$tenant->name}",
            ]
        );

        return back()
            ->with('success', 'API token created. Copy it now — it will not be shown again.')
            ->with('new_api_token', $plain);
    }

    public function revoke(int $tenantId, int $tokenId)
    {
        $tenant = Tenant::query()->findOrFail($tenantId);
        $token = TenantApiToken::query()
            ->where('tenant_id', $tenant->id)
            ->findOrFail($tokenId);

        if ($token->status === 'revoked') {
            return back()->with('error', 'Token is already revoked.');
        }

        $token->revoke();

        $actor = Auth::guard('super_admin')->user();

        AuditLog::record(
            'tenant.api_token_revoked',
            $tenant->id,
            'super_admin',
            $actor?->id,
            $actor?->name,
            [
                'subject_type' => 'tenant_api_token',
                'subject_id'   => $token->id,
                'description'  => "Revoked API token \"{$token->name}\" for {$tenant->name}",
            ]
        );

        return back()->with('success', 'API token revoked.');
    }
}
