        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('upwork.client.account-access') }}" class="leadform"
                        id="form1">
                        @csrf
                        <div class="row">
    ...
    <script>
        $(document).on("click", ".banUser, .unbanUser", function() {
            ...
            if (confirm(`Are you sure you want to ${actionText} this user?`)) {
                $.ajax({
                    url: "{{ route('upwork.client.updateStatus') }}", // Ensure the route exists
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        user_id: userId,
        ...
        $(document).on("click", ".deleteUser", function() {
            ...
            if (confirm("Are you sure you want to delete this user?")) {
                $.ajax({
                    url: "{{ route('upwork.client.delete') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        user_id: userId
