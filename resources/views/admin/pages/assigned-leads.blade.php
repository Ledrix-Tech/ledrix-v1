...
@section('title', 'Admin | Assigned Leads')
                        ...
                        @else
                            @foreach ($leads as $i => $lead)
                                ...
                                <tr>
                                    ...
                                    @php
                                        // because we filtered assignments in the controller, this will be the PM row for this seller
                                        $assignment = $lead->assignments->first();
                                        $pm = $assignment?->assignee; // Seller model who is the PM
                                    @endphp
                                    <td>
                                        @if ($pm)
                                            <div class="fw-semibold">{{ $pm->name }}</div>
                                            <div class="text-muted small">{{ $pm->email ?? $pm->sudo_name }}</div>
                                            <div class="badge bg-info-subtle text-info small">
                                                Assigned •
                                                {{ \Illuminate\Support\Str::replace('_', ' ', $assignment->status) }}
                                            </div>
                                        @else
                                            {{-- Fallback: not assigned -> show original owner --}}
                                            <div class="fw-semibold">{{ $lead->seller->name ?? '—' }}</div>
                                            <div class="text-muted small">
                                                {{ $lead->seller->email ?? ($lead->seller->sudo_name ?? '—') }}</div>
                                        @endif
                                    </td>
                                    {{-- <td>
                                    ...
                                    <td class="blurred">
                                        ...
                                        <div class="text-muted small ">
                                            <a
                                                href="mailto:{{ $lead->phone ?? '—' }}">{{ Str::mask($lead->phone, '*', 3) ?? '—' }}</a>
                                        </div>
                                    </td>
                                    <td>
                                        <div>
