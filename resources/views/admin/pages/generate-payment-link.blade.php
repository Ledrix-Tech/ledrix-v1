...
@section('title', 'Admin | Generate Payment Link')
...
@section('admin-content')
    ...
    <div class="crm-paylink-layout">
        <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('admin.leads.get') }}"
                            ...
                            <div class="col-md-6">
                                ...
                                @if ($isOrder)
                                    <div class="crm-field-hint">
                                        Due: {{ number_format($dueCents / 100, 2) }} {{ $order->currency }}
                                        @if (! $hasMilestones)
                                            · Full balance required (milestones not enabled for this tenant)
                                        @endif
                                    </div>
                                @elseif (! $hasMilestones)
                                    <div class="crm-field-hint">Auto-matches total — partial payments require milestone billing on the tenant plan</div>
                                @else
